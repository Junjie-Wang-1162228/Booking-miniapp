...
- [ ] 运行 `pnpm miniapp:sync-private-config`，在微信开发者工具中打开小程序构建目录 `apps/miniapp/dist`，不要打开源码目录 `apps/miniapp`；确认本地 `apps/miniapp/project.private.config.json` 已配置真实 AppID；运行 `pnpm ops:manual-test:readiness` 确认构建包 API 地址不是 `localhost`，且构建包 API 的 `/health` 可访问。
...
- [ ] 会员微信号继续走微信授权登录和绑定码流程，不使用账号密码登录。
