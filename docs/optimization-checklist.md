...
- [x] 新增 `pnpm ops:manual-test:handoff` 中文 Markdown 交接报告：汇总真机验收状态、DevTools 打开目录、优先顺序、手工验收分组、视觉截图诊断、`visualQaNext` 下一台设备和截图保存路径、下一条截图命令、可复制命令和发布阻断项，方便发给协作者继续测试且不打开微信开发者工具。
...
- [x] 方案二品牌深化：预约页、账户页和课程详情页补齐统一的训练馆视觉锚点，增加灰色训练垫、黑色分道线、橙红落点和页面业务状态文案，让核心用户路径不只首页有拳馆风格。
- [x] 运营端小程序同步轻量品牌样式：保留排课、名单、消课、会员绑定的工具密度，同时统一指标卡、表单、按钮和预约卡的红橙品牌视觉。
- [x] 方案二窄屏兜底：品牌场馆信息条、训练条、开发会员切换、课程操作按钮、预约取消按钮、运营端表单和操作区在小屏设备下改为纵向或双列布局，降低 iPhone SE 等窄屏溢出风险，并纳入 `pnpm miniapp:brand-ui:test` 静态门禁。
