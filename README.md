...
`pnpm ops:manual-test:readiness` 会执行严格本地状态检查并输出 `manual-test-readiness` JSON，把本地预览、strict 环境门禁、视觉截图矩阵和手工验收清单汇总到一起。它不打开微信开发者工具；只有本地预览和 strict 门禁通过时才表示可以开始真实微信人工验收，视觉截图和完整 checklist 仍作为发布前剩余项继续展示。输出里的 `nextHumanAction` 会在本地门禁已通过时跳过重复的本地启动步骤，直接提示继续真实微信登录准备。
